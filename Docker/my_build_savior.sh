#!/bin/bash
#Purpose of copying this file: leave git repo structure intact.
PROJ=savior-source
SOFTWARE_DIR=~/git/software-savior
WORK_DIR=~/git/
...
################Installation script
    ...
    patch -p0 < $WORK_DIR/$PROJ/patches/clang-3.6.patch
    ...
    #installing uclibc++
    cd $WORK_DIR/$PROJ/klee-uclibc++
    ./configure --with-llvm-build-dir=/usr/local/ --with-klee-src-dir=$WORK_DIR/$PROJ/KLEE
    make -j$(nproc)
    cd ..
    ...
    #installing klee-3.6
    rm -rf $WORK_DIR/$PROJ/KLEE/klee-build
    cd $WORK_DIR/$PROJ/KLEE
    mkdir klee-build
    cd klee-build
    echo "NOTE: you might need to rebuild libboost for C++ ABI compatibility on Ubuntu 16.04"
    cmake -DLLVM_CONFIG_BINARY=/usr/local/bin/llvm-config -DENABLE_SOLVER_STP=ON -DENABLE_SOLVER_Z3=ON -DENABLE_POSIX_RUNTIME=ON -DENABLE_KLEE_UCLIBCXX=ON -DKLEE_UCLIBCXX_PATH=$WORK_DIR/$PROJ/klee-uclibc++ -DENABLE_KLEE_UCLIBC=ON -DKLEE_UCLIBC_PATH=$SOFTWARE_DIR/klee-uclibc  -DENABLE_UNIT_TESTS=OFF -DENABLE_SYSTEM_TESTS=OFF -DENABLE_TCMALLOC=ON ..
    make -j$(nproc)
    make install
